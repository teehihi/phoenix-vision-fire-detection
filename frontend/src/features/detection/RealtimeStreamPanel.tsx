    ...
    <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Frame đã qua YOLOv11, OpenCV, risk scoring và temporal smoothing.</p>
        </div>
          ...
          {imageSrc ? (
            ...
            <div className="grid h-full place-items-center p-8 text-center text-slate-300">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">Chạy AI service ở port 8001 rồi mở lại trang này.</p>
              </div>
            </div>
          )}
...
function DetectionSummary({ frame }: { frame: ProcessedFrameMessage | null }) {
  ...
  const personCount = detections.filter((item) => item.label === 'person').length;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        ...
        <ObjectCount label="Person" value={personCount} tone="text-cyan-700 bg-cyan-50" />
      </div>
...
function ObjectCount({ label, value, tone }: { label: string; value: number; tone: string }) {
  const Icon = label === 'Person' ? Users : label === 'Fire' ? AlertTriangle : Activity;
  return (
