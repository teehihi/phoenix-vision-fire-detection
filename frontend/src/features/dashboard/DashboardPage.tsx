    ...
    <CinematicShell riskLevel="HIGH">
      ...
      <motion.header variants={variants} className="cinematic-card overflow-hidden rounded-2xl border border-white/80 bg-white/75 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur-xl">
        <div className="flex flex-wrap items-start justify-between gap-5">
          ...
          <motion.div
            className="rounded-2xl border border-red-100 bg-red-50/80 px-5 py-4 text-right shadow-sm"
            animate={{ boxShadow: ['0 0 0 rgba(239,68,68,0)', '0 0 32px rgba(239,68,68,0.22)', '0 0 0 rgba(239,68,68,0)'] }}
  ...
  return (
    <div className="rounded-2xl border border-white/80 bg-white/80 p-5 shadow-sm backdrop-blur-xl">
      ...
      <div className="space-y-3">
        {timeline.map((item) => (
          <motion.div key={`${item.time}-${item.title}`} whileHover={{ x: 4 }} className="flex gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-3">
            <motion.div
              className={`mt-1 h-3 w-3 rounded-full ${item.level === 'critical' ? 'bg-fuchsia-600' : item.level === 'high' ? 'bg-red-500' : 'bg-amber-400'}`}
              animate={{ scale: [1, 1.35, 1], opacity: [0.7, 1, 0.7] }}
              transition={{ duration: item.level === 'critical' ? 0.9 : 1.6, repeat: Infinity, ease: 'easeInOut' }}
            />
            <div>
